...
def initialize_database():
    ...
    organization_id = get_valid_organization_id(conn)
...
def save_certificate(document_id, certificate_data, user_id, organization_id=None):
    ...
    try:
        ...
        # For PostgreSQL - adapt to Supabase schema
            ...
            try:
                # Try to find the document by ID or name
                cursor.execute(
                ...
                # If no result, create a new document in production mode
                if not document_result and os.getenv('NODE_ENV') == 'production':
                    # Create a document entry for this filename
                    doc_uuid = str(uuid.uuid4())
                    try:
                        cursor.execute(
                ...
                # Handle the case where the document_id is not a valid UUID
                print(f"Error checking for document: {e}")
                document_id_uuid = None
            
            # Handle both string and UUID user IDs
